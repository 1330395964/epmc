...
public class ExcelParser {
    ...
    public static void main(String[] args) throws IOException, ParseException, InvalidFormatException {
        File tempFile = new File( "/Users/mac/Desktop/123456.xlsx");
//传入一个路径产生流再将流传入工具类，返回解析对象，Excel的所有数据就被解析到List<String[]> 里面，遍历list任由你处置。
        ...
        for (int i=0; i<datas.size();i++) {
            if((datas.get(i)[20].startsWith("2016") || datas.get(i)[20].startsWith("2017")
                    || datas.get(i)[20].startsWith("2018")|| datas.get(i)[20].startsWith("2019"))
                && (datas.get(i)[13].startsWith("大数据") || datas.get(i)[13].startsWith("信息与电子"))){
                list.add(datas.get(i));
            }
        }
        String[] strings = datas.get(datas.size() - 1);
        File file = new File("/Users/mac/Desktop/123456.txt");
